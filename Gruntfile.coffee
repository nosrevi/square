...
module.exports = (grunt) ->
    ...
    grunt.registerTask "server", (target) ->
        return grunt.task.run(["build", "express:prod", "open"])  if target is "dist"
        grunt.task.run ["clean:server", "concurrent:server", "express:dev", "open", "watch"]

    grunt.registerTask "lessServer", (target) ->
        return grunt.task.run(["buildLess", "open"])  if target is "dist"
        grunt.task.run ["clean:server", "concurrent:lessServer", "open", "watch"]
